"""Pin dbt1/macros/iceberg_adapter_overrides.sql against the INSTALLED dbt-duckdb.
...
THE GUARD-ONLY TEST RUNS EVERYWHERE; THE TWO FALLBACK TESTS NEED dbt-duckdb INSTALLED and
skip without it. requirements/dev.txt deliberately installs no dbt, so the free unit job
checks the guards and ci.yml's `gating` matrix — which does install an adapter — is where the
upstream comparison actually runs. Do not add dbt to dev.txt to make these unskip.
...
OVERRIDES = (
    pathlib.Path(__file__).resolve().parents[1] / "dbt1" / "macros" / "iceberg_adapter_overrides.sql"
)


def _installed_adapters_sql() -> str:
    inc = pytest.importorskip(
        "dbt.include.duckdb",
        reason="dbt-duckdb is not installed; the fallback comparison runs in ci.yml's gating job",
    )
