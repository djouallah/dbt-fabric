...
| `dwh` | `dbt-fabric` | Fabric Warehouse | distributed (MPP) | Warehouse tables (T-SQL) |
...
| dwh | `fct_summary`'s rebuild is decided by the **runner**: `check_new_daily` before the build, `rebuild_summary: true` (delete+insert) when daily files are landing. The model cannot decide it itself because its strategy is a parse-time config |
...
| all | `fct_summary`'s increment mechanics come in three shapes — an insert-only merge behind a has-new-daily probe (DuckDB family), a rebuild-dates window merge (spark), a runner-decided delete+insert (dwh) — because that is what each merge implementation allows. Same table out |
