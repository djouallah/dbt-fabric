...
| iceberg | **credential vending only** — the catalog bearer authorises the catalog, and every data file is read and written on a credential the catalog hands back per table. It needs `--pre duckdb`: the vended `adls.sas-token.*` is unusable before [duckdb-iceberg#1331](https://github.com/duckdb/duckdb-iceberg/pull/1331). `stage_create_tables: 0` survives it anyway, because OneLake vends nothing on `createTable` |
